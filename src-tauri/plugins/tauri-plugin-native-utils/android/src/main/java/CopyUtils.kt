package com.fallak.plugin.native_utils
...
fun copyUri(
    ...
    if (totalBytes > 0 && copiedBytes != totalBytes) {
        throw IOException("File truncated during copy: copied $copiedBytes bytes but expected $totalBytes bytes. Ensure your phone has enough free storage space.")
    }
...
private fun copyUriTreeWithProgress(
    ...
    if (totalBytes > 0 && copiedBytes != totalBytes) {
        throw IOException("Folder truncated during copy: copied $copiedBytes bytes but expected $totalBytes bytes. Ensure your phone has enough free storage space.")
    }

...
pub fn is_context_menu_registered() -> anyhow::Result<bool> {
    ...
    let path = "Software\\Classes\\*\\shell\\Send with Fall ak Smart Transfer\\command";
...
pub fn register_context_menu() -> anyhow::Result<()> {
    ...
    // Register for Files (*)
    write_registry_key("*", "Send with Fall ak Smart Transfer", exe_str, &icon_path, "\"%1\"")?;

    // Register for Directories
    write_registry_key(
        "Directory",
        "Send with Fall ak Smart Transfer",
        exe_str,
        &icon_path,
        "\"%1\"",
    )?;
...
pub fn unregister_context_menu() -> anyhow::Result<()> {
    remove_registry_key("*", "Send with Fall ak Smart Transfer")?;
    remove_registry_key("Directory", "Send with Fall ak Smart Transfer")?;
    remove_registry_key("Directory\\Background", "Send with Fall ak Smart Transfer")?;
    notify_icon_change();
    Ok(())
}
